...
### 3-5. Donchian + ATRトレーリング（日足スイング / 2026-07-02追加）

**コンセプト**: 中期の空白を埋める日足トレンドフォロー。手数料・滑り・レイテンシの影響が小さく、小資金・EU VPSに適する。

| パラメータ | 値 | 備考 |
|-----------|-----|------|
| エントリー足 | 日足 | UTC 0:00区切り |
| チャネル期間 | 20日 | 直前20日（当日足を含まない）の高値/安値 |
| エントリー | 終値がチャネル上抜け→ロング / 下抜け→ショート | 成行（taker）想定 |
| 決済 | ATR(14) × 3 トレーリングストップのみ | 固定TPなし。毎日クローズで切り上げ/切り下げ |
| 最大同時ポジション | 1 | — |

### 3-6. 共通フィルター（2026-07-02追加）

| フィルター | 対象 | 内容 |
|-----------|------|------|
| ADXレジームフィルター | 逆張り系（rsi30 / rsi30_fibo / bb_rsi / pivot系） | ADX(14) ≥ 25（トレンド相場）ではエントリーしない。戦略内部で判定 |
| 日足200EMA方向フィルター | 全directional戦略 | 日足終値 > 200EMA ならロングのみ、下ならショートのみ。main.py側で判定。`TREND_FILTER_ENABLED=0` で無効化（バックテストには非適用） |
